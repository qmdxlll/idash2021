...
const int d_input = 16*16;
const int d_fc0 = 16;
const int d_output = 4;
const double alpha = SECALPHA;
const int spc_msg = 1000;   //ciphertext space
//TLWE
    ...
    for(int k=0;k<2;k++){
        for (int32_t i = 0; i < len1; ++i) {//256
            for(int32_t j = 0; j < len2/2; ++j){//1000
                float plain = (float)input[j+k*len2/2][i]/100;
...
int main()
{
	...
	ifstream fc0("txt/fc0w.txt");
	readweight0(fc0, fc0w);//16*256
	fc0.close();
    ifstream fc1("txt/fc1w.txt");
	readweight1(fc1, fc1w);//4*16
	fc1.close();
    endread =clock();
...
//  enc
    ...
    TLweSample *enc_input = new_TLweSample_array(input_width*2,tlweparams);
